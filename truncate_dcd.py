...
with MDAnalysis.Writer(os.path.join(args.data_dir, "test1.dcd"), u.atoms.n_atoms) as W:
    for ts in u.trajectory[-200:-150]:
        W.write(protein)
with MDAnalysis.Writer(os.path.join(args.data_dir, "test2.dcd"), u.atoms.n_atoms) as W:
    for ts in u.trajectory[-150:-100]:
        W.write(protein)
with MDAnalysis.Writer(os.path.join(args.data_dir, "test3.dcd"), u.atoms.n_atoms) as W:
    for ts in u.trajectory[-100:-50]:
        W.write(protein)
with MDAnalysis.Writer(os.path.join(args.data_dir, "test4.dcd"), u.atoms.n_atoms) as W:
